...
#define CHAT_IDTEST "xxxxxxxx"  //@Y3Project_bot
...
//hard plastic
byte pin_rows[Row_Keypad] = {16, 46, 3, 18};     //row pinouts of the keypad
byte pin_column[Col_Keypad] = {17, 15, 8};  //column pinouts of the keypad
...
//TelegramBot
const unsigned long BOT_MTBS = 15000; // mean time between scan messages
...
//testing area below
const int web_manager=45;
const int tamperTrigger=5;
bool isTampered=false;

char encrypted_pass[4];
...
void setup() {
  ...
  */
  ...
  pinMode(tamperTrigger, INPUT_PULLUP);
...
void loop() {
  ...
  if(moduleData.isAlarm == false && isSet == false && isTampered==false){
    is_Alarm=false;
    isTampered=false;
    lcd.setCursor(5, 1);
    lcd.print("        ");
  ...
  if(digitalRead(tamperTrigger)== 0){
    //is_Alarm=true;
    isTampered=true;
    is_Alarm=true;
    lcd.setCursor(5, 1);
    ...
    start_WiFi();
    bot.sendMessage(CHAT_IDTEST, "System has been Tampered");
    end_WiFi();
  }
  else{
    isTampered=false;
...
void F_Password_input(){
  ...
  if(key){  
    ...
    if(key== '*'){
      ...
      if(strcmp(encrypted_pass,Saved_Password)== 0){
        //encrypted_pass== Saved_Password
        //strcmp(encrypted_pass,Saved_Password);
...
void F_EncryptPassword(){
  ...
  int Encrpt_Key[10]={3,3,3,3}; //key to shift input using index 
 ...
 // lcd.print(Encrypted_Password);
  ...
  encrypted_pass[0]=Encrypted_Password[0];
  encrypted_pass[1]=Encrypted_Password[1];
  encrypted_pass[2]=Encrypted_Password[2];
  encrypted_pass[3]=Encrypted_Password[3];
  //Remove
  /*
  lcd.setCursor(9, 4);
...
void F_fail_count_check(){
  ...
  if(fail_count>=3){
    ...
    bot.sendMessage(CHAT_IDTEST, "exceded failed inputs", "");
    delay(1000);
    fail_count=0;
  }
}
...
void check_telegram(){
  if (millis() - bot_lasttime > BOT_MTBS){  //if over the time past since last check
    lcd.setCursor(0, 3); //carlow issue who where delay with slow internet is 
    lcd.print("#");
    start_WiFi();
...
void reset_log_flag(){
  if(isSet == false && isLogSaved == true){
